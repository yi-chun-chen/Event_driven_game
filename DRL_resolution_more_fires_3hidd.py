...
class UAV_fire_extinguish(object):
  ...
  t_fail = [0.000,0.000]
  t_emit = [1.0,1.0]
  l_fire = [4,   10,   30,   42,     49,    60]
  r_fire = [5.0, 10.0, 10.0, 100.0,  5.0,  5.0]
  e_fire = [[0.9,0.9],
            [0.5,0.9],
            [0.5,0.9],
            [1.0,0.9],
...
def transition_model(cart_product,a_joint,UAV_fire_extinguish):
  ...
  initial_state = list((UAV_fire_extinguish.u_loca[0],UAV_fire_extinguish.u_loca[1],1,1,1,1,1,1))
...
def transition_sample(current_state, # location is one-dimensional
  ...
  next_state_coor = [xp0,yp0,xp1,yp1,next_state[2],next_state[3],next_state[4],next_state[5],next_state[6],next_state[7]]
...
def samples_by_random_action(n_init_pool,s_init,UAV_fire_extinguish):
  ...
  input_number = UAV_fire_extinguish.n_uav * 2 + UAV_fire_extinguish.n_fire + 1

  s0_pool = np.zeros((n_init_pool,input_number),float)
  a0_pool = np.zeros((n_init_pool,5),float)
  r0_pool = np.zeros((n_init_pool,1),float)
  s0p_pool = np.zeros((n_init_pool,input_number),float)

  s1_pool = np.zeros((n_init_pool,input_number),float)
  a1_pool = np.zeros((n_init_pool,5),float)
  r1_pool = np.zeros((n_init_pool,1),float)
  s1p_pool = np.zeros((n_init_pool,input_number),float)
  ...
  s_current_coor = [x0_init,y0_init,x1_init,y1_init,s_init[2],s_init[3],s_init[4],s_init[5],s_init[6],s_init[7]]
  ...
  for i_event in range(n_init_pool):
    ...
    s0_pool[i_event,:] = last_info_0_norm
    s0p_pool[i_event,:] = next_info_0_norm
    a0_pool[i_event,a0] = 1.0
    r0_pool[i_event,0] = reward

    s1_pool[i_event,:] = last_info_1_norm
    s1p_pool[i_event,:] = next_info_1_norm
...
def samples_by_random_action_fix_one(n_init_pool,s_init,be_fixed,UAV_fire_extinguish):
  ...
  for i_event in range(n_init_pool):
    ...
    if be_fixed == 1:
      ...
      a0 = random.randint(0,4)
      a1 = es_greedy(sess.run(Q_a1, feed_dict={last_info_a1: [last_info_1_norm]}),0.0)

    else: #be_fixed == 0:
      a1 = random.randint(0,4)
      a0 = es_greedy(sess.run(Q_a0, feed_dict={last_info_a0: [last_info_0_norm]}),0.0)
...
def visualize_scenario_indp(initial_state,h_print,r_explore,UAV_fire_extinguish):
  ...
  for h in range(h_print):
    action_chosen_1 = es_greedy(sess.run(Q_a1, feed_dict={last_info_a1: [last_info_1_norm]}),r_explore)
    action_chosen_0 = es_greedy(sess.run(Q_a0, feed_dict={last_info_a0: [last_info_0_norm]}),r_explore)
...
def visualize_scenario_indp_softmax(initial_state,h_print,T,UAV_fire_extinguish):
  ...
  for h in range(h_print):
    action_chosen_1 = softmax(sess.run(Q_a1, feed_dict={last_info_a1: [last_info_1_norm]}),T)
    action_chosen_0 = softmax(sess.run(Q_a0, feed_dict={last_info_a0: [last_info_0_norm]}),T)
...
last_info_a0 = tf.placeholder(tf.int32,[None,input_number])
next_info_a0 = tf.placeholder(tf.int32,[None,input_number])
actions_a0 = tf.placeholder(tf.float32,[None,5])
rewards_a0 = tf.placeholder(tf.float32,[None,1])

W1_train_a0 = tf.placeholder(tf.float32,[input_number,n_hidd])
b1_train_a0 = tf.placeholder(tf.float32,[1,n_hidd])
W2_train_a0 = tf.placeholder(tf.float32,[n_hidd,n_hidd])
b2_train_a0 = tf.placeholder(tf.float32,[1,n_hidd])
W3_train_a0 = tf.placeholder(tf.float32,[n_hidd,n_hidd])
b3_train_a0 = tf.placeholder(tf.float32,[1,n_hidd])
W4_train_a0 = tf.placeholder(tf.float32,[n_hidd,5])
b4_train_a0 = tf.placeholder(tf.float32,[1,5])

last_info_a1 = tf.placeholder(tf.int32,[None,input_number])
next_info_a1 = tf.placeholder(tf.int32,[None,input_number])
actions_a1 = tf.placeholder(tf.float32,[None,5])
rewards_a1 = tf.placeholder(tf.float32,[None,1])

W1_train_a1 = tf.placeholder(tf.float32,[input_number,n_hidd])
b1_train_a1 = tf.placeholder(tf.float32,[1,n_hidd])
W2_train_a1 = tf.placeholder(tf.float32,[n_hidd,n_hidd])
b2_train_a1 = tf.placeholder(tf.float32,[1,n_hidd])
W3_train_a1 = tf.placeholder(tf.float32,[n_hidd,n_hidd])
b3_train_a1 = tf.placeholder(tf.float32,[1,n_hidd])
W4_train_a1 = tf.placeholder(tf.float32,[n_hidd,5])
b4_train_a1 = tf.placeholder(tf.float32,[1,5])
...
layer_1_a0   = add_layer(last_info_a0 , input_number,      n_hidd, activation_function = tf.nn.relu)
layer_2_a0   = add_layer(layer_1_a0[0], n_hidd,            n_hidd, activation_function = tf.nn.relu)
layer_3_a0   = add_layer(layer_2_a0[0], n_hidd,            n_hidd, activation_function = tf.nn.relu)
layer_out_a0 = add_layer(layer_3_a0[0], n_hidd,                 5, activation_function = None)
Q_a0 = layer_out_a0[0]

layer_c1_a0 = copy_layer(next_info_a0, W1_train_a0, b1_train_a0, activation_function = tf.nn.relu)
layer_c2_a0 = copy_layer(layer_c1_a0,  W2_train_a0, b2_train_a0, activation_function = tf.nn.relu)
layer_c3_a0 = copy_layer(layer_c2_a0,  W3_train_a0, b3_train_a0, activation_function = tf.nn.relu)
Q_next_a0 =   copy_layer(layer_c3_a0,  W4_train_a0, b4_train_a0, activation_function = None)

##### Agent 1's NN #####

layer_1_a1   = add_layer(last_info_a1,    input_number,      n_hidd, activation_function = tf.nn.relu)
layer_2_a1   = add_layer(layer_1_a1[0],   n_hidd,            n_hidd, activation_function = tf.nn.relu)
layer_3_a1   = add_layer(layer_2_a1[0],   n_hidd,            n_hidd, activation_function = tf.nn.relu)
layer_out_a1 = add_layer(layer_3_a1[0],   n_hidd,                 5, activation_function = None)
Q_a1 = layer_out_a1[0]

layer_c1_a1 = copy_layer(next_info_a1, W1_train_a1, b1_train_a1, activation_function = tf.nn.relu)
layer_c2_a1 = copy_layer(layer_c1_a1,  W2_train_a1, b2_train_a1, activation_function = tf.nn.relu)
layer_c3_a1 = copy_layer(layer_c2_a1,  W3_train_a1, b3_train_a1, activation_function = tf.nn.relu)
Q_next_a1 =   copy_layer(layer_c3_a1,  W4_train_a1, b4_train_a1, activation_function = None)
...
initializer = get_weight_initializer('relu')

W1_for_feed_train_a0 = sess.run(initializer((input_number, n_hidd)))
b1_for_feed_train_a0 = np.ones((1,n_hidd),float) * 0.1
W2_for_feed_train_a0 = sess.run(initializer((n_hidd, n_hidd)))
b2_for_feed_train_a0 = np.ones((1,n_hidd),float) * 0.1
W3_for_feed_train_a0 = sess.run(initializer((n_hidd, n_hidd)))
b3_for_feed_train_a0 = np.ones((1,n_hidd),float) * 0.1
W4_for_feed_train_a0 = sess.run(initializer((n_hidd, 5)))
b4_for_feed_train_a0 = np.ones((1,5),float) * 0.1

W1_for_feed_train_a1 = sess.run(initializer((input_number, n_hidd)))
b1_for_feed_train_a1 = np.ones((1,n_hidd),float) * 0.1
W2_for_feed_train_a1 = sess.run(initializer((n_hidd, n_hidd)))
b2_for_feed_train_a1 = np.ones((1,n_hidd),float) * 0.1
W3_for_feed_train_a1 = sess.run(initializer((n_hidd, n_hidd)))
b3_for_feed_train_a1 = np.ones((1,n_hidd),float) * 0.1
W4_for_feed_train_a1 = sess.run(initializer((n_hidd, 5)))
b4_for_feed_train_a1 = np.ones((1,5),float) * 0.1
...
outcome = samples_by_random_action(n_init_pool,[0,8**2-1,1,1,1,1,1,1],UAV_8_by_8)
...
h_train_step = 400 * h_step_for_gradient
h_grad = 100
r_explore = 0.9
n_upper_size = 50000
it_time = 2
...
for task in range(1):
  ...
  current_state = [0,size**2-1,1,1,1,1,1,1]
  next_state = 0
  last_info_0 = [0,0,size-1,size-1,1,1,1,1,1,1,1]
  last_info_1 = [0,0,size-1,size-1,1,1,1,1,1,1,1]
  ...
  for iteration_times in range(it_time):
    ...
    if iteration_times == 0:
      UAV_task.e_fire[3][0] = 0.8
      print("rate is ", UAV_task.e_fire[3][0])
    elif iteration_times == 1:
      UAV_task.e_fire[3][0] = 0.4
      print("rate is ", UAV_task.e_fire[3][0])
    elif iteration_times == 2:
      UAV_task.e_fire[3][0] = 0.2
      print("rate is ", UAV_task.e_fire[3][0])
    else:
      UAV_task.e_fire[3][0] = 0.1
      print("rate is ", UAV_task.e_fire[3][0])
    ...
    if iteration_times + task != 0 :
      ...
      outcome = samples_by_random_action_fix_one(n_init_pool,[0,size**2 - 1,1,1,1,1,1,1],1,UAV_task)
    ...
    for h in range(h_train_step):
      ...
      if h % h_step_for_gradient == 0:
        ...
        for i in range(h_grad):
          ...
          numeric_loss_a0, _ = sess.run([loss_a0,train_step_a0],feed_dict={last_info_a0: s0_batch_0,
                                                                           ...
                                                                           W3_train_a0: W3_for_feed_train_a0,
                                                                           b3_train_a0: b3_for_feed_train_a0,
                                                                           W4_train_a0: W4_for_feed_train_a0,
                                                                           b4_train_a0: b4_for_feed_train_a0,
      ...
      action_chosen_0 = es_greedy(sess.run(Q_a0, feed_dict={last_info_a0: [last_info_0_norm]}), 0.1 + r_explore * (1 - float(h)/h_train_step) )
      if iteration_times + task == 0:
        action_chosen_1 = random.randint(0,4)
      else:
        action_chosen_1 = es_greedy(sess.run(Q_a1, feed_dict={last_info_a1: [last_info_1_norm]}),0.0)
      ...
      if h%(20*h_step_for_gradient) == 0: print(h,numeric_loss_a0,last_info_0_norm)
      ...
      s0_array_0 = np.vstack([s0_array_0, last_info_0_norm])
      sp0_array_0 = np.vstack([sp0_array_0, next_info_0_norm])
      r0_array_0 = np.vstack([r0_array_0, reward_immed])

      s1_array_0 = np.vstack([s1_array_0, last_info_1_norm])
      sp1_array_0 = np.vstack([sp1_array_0, next_info_1_norm])
      ...
      if h% (h_step_for_gradient * 50) == 0:
        W1_for_feed_train_a0 = sess.run(layer_1_a0[1], feed_dict={last_info_a0: [last_info_0_norm]})
        b1_for_feed_train_a0 = sess.run(layer_1_a0[2], feed_dict={last_info_a0: [last_info_0_norm]})
        W2_for_feed_train_a0 = sess.run(layer_2_a0[1], feed_dict={last_info_a0: [last_info_0_norm]})
        b2_for_feed_train_a0 = sess.run(layer_2_a0[2], feed_dict={last_info_a0: [last_info_0_norm]})
        W3_for_feed_train_a0 = sess.run(layer_3_a0[1], feed_dict={last_info_a0: [last_info_0_norm]})
        b3_for_feed_train_a0 = sess.run(layer_3_a0[2], feed_dict={last_info_a0: [last_info_0_norm]})
        W4_for_feed_train_a0 = sess.run(layer_out_a0[1], feed_dict={last_info_a0: [last_info_0_norm]})
        b4_for_feed_train_a0 = sess.run(layer_out_a0[2], feed_dict={last_info_a0: [last_info_0_norm]})
    ...
    visualize_scenario_indp([0,size**2-1,1,1,1,1,1,1],50,0.0,UAV_task)
    print("================")
    visualize_scenario_indp([0,size**2-1,1,1,1,1,1,1],50,0.0,UAV_task)
    print("================")
    visualize_scenario_indp([0,size**2-1,1,1,1,1,1,1],50,0.0,UAV_task)
    ...
    if iteration_times == 0:
      UAV_task.e_fire[3][0] = 0.6
      print("rate is ", UAV_task.e_fire[3][0])
    elif iteration_times == 1:
      UAV_task.e_fire[3][0] = 0.3
      print("rate is ", UAV_task.e_fire[3][0])
    elif iteration_times == 2:
      UAV_task.e_fire[3][0] = 0.1
      print("rate is ", UAV_task.e_fire[3][0])
    ...
    if iteration_times + task != -1 :
      ...
      outcome = samples_by_random_action_fix_one(n_init_pool,[0,size**2 - 1,1,1,1,1,1,1],0,UAV_task)
    ...
    for h in range(h_train_step):
      ...
      if h % h_step_for_gradient == 0:
        for i in range(h_grad):
          ...
          numeric_loss_a1, _ = sess.run([loss_a1,train_step_a1],feed_dict={last_info_a1: s1_batch_1,
                                                                           ...
                                                                           W3_train_a1: W3_for_feed_train_a1,
                                                                           b3_train_a1: b3_for_feed_train_a1,
                                                                           W4_train_a1: W4_for_feed_train_a1,
                                                                           b4_train_a1: b4_for_feed_train_a1,
                                                                           learning_rate: learning_rate_given})
      ...
      action_chosen_1 = es_greedy(sess.run(Q_a1, feed_dict={last_info_a1: [last_info_1_norm]}),0.1 + r_explore * (1 - float(h)/h_train_step))
      action_chosen_0 = es_greedy(sess.run(Q_a0, feed_dict={last_info_a0: [last_info_0_norm]}),0.0)
      ...
      s0_array_1 = np.vstack([s0_array_1, last_info_0_norm])
      sp0_array_1 = np.vstack([sp0_array_1, next_info_0_norm])
      r0_array_1 = np.vstack([r0_array_1, reward_immed])

      s1_array_1 = np.vstack([s1_array_1, last_info_1_norm])
      sp1_array_1 = np.vstack([sp1_array_1, next_info_1_norm])
      ...
      if h% (h_step_for_gradient * 50) == 0:
        W1_for_feed_train_a1 = sess.run(layer_1_a1[1], feed_dict={last_info_a1: [last_info_1_norm]})
        b1_for_feed_train_a1 = sess.run(layer_1_a1[2], feed_dict={last_info_a1: [last_info_1_norm]})
        W2_for_feed_train_a1 = sess.run(layer_2_a1[1], feed_dict={last_info_a1: [last_info_1_norm]})
        b2_for_feed_train_a1 = sess.run(layer_2_a1[2], feed_dict={last_info_a1: [last_info_1_norm]})
        W3_for_feed_train_a1 = sess.run(layer_3_a1[1], feed_dict={last_info_a1: [last_info_1_norm]})
        b3_for_feed_train_a1 = sess.run(layer_3_a1[2], feed_dict={last_info_a1: [last_info_1_norm]})
        W4_for_feed_train_a1 = sess.run(layer_out_a1[1], feed_dict={last_info_a1: [last_info_1_norm]})
        b4_for_feed_train_a1 = sess.run(layer_out_a1[2], feed_dict={last_info_a1: [last_info_1_norm]})
