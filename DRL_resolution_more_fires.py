...
W1_for_feed_train_a0 = np.random.rand(input_number,n_hidd) * 0.5 - np.ones((input_number,n_hidd),float)
...
W1_for_feed_train_a1 = np.random.rand(input_number,n_hidd) * 0.5 - np.ones((input_number,n_hidd),float)
...
h_train_step = 300 * h_step_for_gradient
h_grad = 100
r_explore = 0.4
n_upper_size = 50000
it_time = 2
...
for task in range(1):
  ...
  for iteration_times in range(it_time):
    ...
    if iteration_times == 0:
      UAV_task.e_fire[3][0] = 0.5
      print("rate is ", UAV_task.e_fire[3][0])
    else:
      UAV_task.e_fire[3][0] = 0.0
      print("rate is ", UAV_task.e_fire[3][0])
    ...
    for h in range(h_train_step):
      ...
      if h% (h_step_for_gradient * 50) == 0:
        ...
        W2_for_feed_train_a0 = sess.run(layer_out_a0[1], feed_dict={last_info_a0: [last_info_0_norm]})
        b2_for_feed_train_a0 = sess.run(layer_out_a0[2], feed_dict={last_info_a0: [last_info_0_norm]})
    ...
    for h in range(h_train_step):
      ...
      if h% (h_step_for_gradient * 50) == 0:
        ...
        W2_for_feed_train_a1 = sess.run(layer_out_a1[1], feed_dict={last_info_a1: [last_info_1_norm]})
        b2_for_feed_train_a1 = sess.run(layer_out_a1[2], feed_dict={last_info_a1: [last_info_1_norm]})
