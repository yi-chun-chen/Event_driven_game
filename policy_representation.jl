...
function simulator_two_policy(
    ...
    for t = 1 : UAV_model.horizon
        ...
        (p1,p2) = (rand(),rand())
        if p1 < UAV_model.t_emit
            u1_last_time = 0
            u1_last_time_message = label_next_state

        end
        if p2 < UAV_model.t_emit
            u2_last_time = 0
            u2_last_time_message = label_next_state
        end

        # add reward
        reward_sum += reward_model(current_state,next_state,UAV_model)
...
function genetic_process_two_uav(
    ...
    for i = 1 : pop_size
        initial_score_u1[i] = fitness_function(20,current_state,initial_pop_u1[i,:],policy_2_fixed,UAV_model)
    end

    # Score for intial population for uav 2
    initial_score_u2 = Array(Float64,pop_size)
    for i = 1 : pop_size
        initial_score_u2[i] = fitness_function(20,current_state,policy_1_fixed,initial_pop_u2[i,:],UAV_model)
    end
    ...
    for t = 1 : iteration_times
        ...
        for i = 1 : round(Int64,pop_size/2)
            ...
            cut_point = 1 + round(Int64,div(h*n_total_s * rand(),1))
